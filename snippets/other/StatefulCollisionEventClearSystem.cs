// Run: cat snippets/other/StatefulCollisionEventClearSystem.cs | unity-cli exec --project ~/Github/bovinelabs-core-internals/BovineLabs --usings "BovineLabs.Core.PhysicsStates,System,System.Reflection,System.Linq"
// Verifies: docs/StatefulCollisionEventClearSystem.md claims
...
// --- PhysicsStates ---
sb.AppendLine("PhysicsStates");
try {
  var t0 = typeof(BovineLabs.Core.PhysicsStates);
  sb.AppendLine($"  Kind: {(t0.IsValueType ? \"struct\" : \"class\")}");
  sb.AppendLine($"  Size: {System.Runtime.InteropServices.Marshal.SizeOf(t0)} bytes");
  check("PhysicsStates exists", t0 != null);
  check("PhysicsStates is ValueType", t0.IsValueType);
...
} catch (System.Exception) { sb.AppendLine("  TYPE NOT FOUND"); check("PhysicsStates exists", false); }

// --- Extensions ---
sb.AppendLine("Extensions");
try {
  var t1 = typeof(BovineLabs.Core.Extensions);
  sb.AppendLine($"  Kind: {(t1.IsValueType ? \"struct\" : \"class\")}");
  sb.AppendLine($"  Size: {System.Runtime.InteropServices.Marshal.SizeOf(t1)} bytes");
  check("Extensions exists", t1 != null);
  check("Extensions is ValueType", t1.IsValueType);
  sb.AppendLine("  Properties:");
